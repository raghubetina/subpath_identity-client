...
gem "subpath_identity-client", github: "raghubetina/subpath_identity-client", tag: "v0.7.0"
```

The two tags differ on purpose — the gems version independently, so
core's latest release is not necessarily this gem's. (CI bundles this
exact block from a clean Gemfile, because a release once shipped
pointing at a core tag that didn't exist.)
...
The marker lives in a separate gem-owned table (`subpath_identity_client_revocations`), not on your profile model, for two reasons. It must be insertable regardless of your profile schema — a `NOT NULL` column or presence validation on a cached field would otherwise turn a revocation into a 500. And it must *outlive* the deleted row: the provider fetch has no per-user lock, so an *older* success can resume after a *newer* revocation and recreate the row; the durable marker (account ids never reuse) is what a later request checks to reap that stray row and stay signed out. `SyncLocalProfile` also rechecks the marker right after applying a fetch, so a revocation that lands mid-fetch is caught in the same request. A marker can still commit *after* that recheck, while the stale request's response is finishing — so be precise about what that window costs. **In the app that recorded the marker**: one request displays the already-revoked profile, and the very next request reaps it via the top-of-action marker check — this app is monotonic. **Cluster-wide**: that stale response also reissued the shared cookie, and its `Set-Cookie` races the revoking response's cookie *deletion* at the browser — two concurrent responses setting one cookie, an ordering RFC 6265 explicitly leaves undefined. If the reissue wins, the browser keeps a cookie the *other* apps go on honoring — but only until its **absolute deadline**, because the reissue never renews it. That's the same exposure as a browser that never visits the discovering app at all (see "Revocation is bounded by the cookie TTL" below), not a new one. No amount of post-write rechecking closes it, and no server-side ordering can decide which of two already-in-flight responses a browser applies last; the invariant this design actually guarantees is the deadline. Both tables come from the `subpath_identity_client:install` migration; an existing install needs to create `subpath_identity_client_revocations`.

### What revocation assumes about your schema

Revocation's row cleanup is a plain `delete_all`, deliberately: no column nulling (so no `NOT NULL` / validation / `lock_version` hazards), it erases the cached PII, and it runs no callbacks. Two consequences follow from "plain DELETE." It is **blocked by an inbound foreign key** — any table that `references` your profile model without `on_delete: :cascade` makes the DELETE raise. And it **skips `dependent:` options and callbacks entirely** (Rails' documented `delete_all` semantics), so dependent rows, attachments, and counter caches aren't handled.

The contract is fail-closed either way: the marker is recorded and the shared cookie cleared **before** cleanup is attempted, and a cleanup failure is **reported and swallowed** — it goes to your app's error reporter (`Rails.error`, severity `:error`, source `"subpath_identity-client"`) rather than raising, because a raise would discard the response along with its cookie-deletion header and turn every request for a definitively gone account into the same 500, forever. A schema that blocks the DELETE therefore shows up in your error tracker while the account still gets, and stays, signed out; the orphaned row's PII lingers until you fix the cleanup, so treat those reports as real work, not noise.

If other tables reference your profile model, either declare those foreign keys `on_delete: :cascade`, or — for cleanup a cascade can't express (polymorphic references, Active Storage attachments, callbacks you actually want) — override the private `remove_local_profile_rows(model, global_user_id)` in the controller that includes `SyncLocalProfile`, with cleanup that fits your schema. Your override runs inside revocation, so keep it something that can't fail for reasons a user request shouldn't 500 on.
...
The same bound is the backstop for the concurrent-response race above: a browser whose cookie-deletion lost the `Set-Cookie` race is simply a browser whose cookie hasn't been cleared yet, and the absolute deadline retires it the same way. Everything about revocation timing in this gem reduces to one sentence — *immediate at the provider, monotonic in the app that observed the typed 410, and bounded by the cookie's absolute deadline everywhere else.*
